...
export default class ItemBase implements IItemBase {
  ...
  constructor(cfg: any) {
    ...
    this._cfg = Object.assign({}, this.getDefaultCfg(), cfg);

    const model = this.get('model'); // data
    let { id } = model;

    const itemType = this.get('type'); // 'node' 'edge' 默认'item'
  ...
  */
  ...
  // node自己实现getDefaultCfg覆盖
  public getDefaultCfg(): IObject {
    return {
      /**
       * id
       * @type {string}
